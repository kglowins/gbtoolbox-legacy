...
public final class TaskTestAgain implements Callable {
	...
	private final TaskResultReprocess res; 
						
	public TaskTestAgain(boolean BT, boolean Bmin, double phi1L, double PhiL, double phi2L,
			double phi1R, double PhiR, double phi2R,
			double zenith, double azimuth, int correlat, double area, 
			double tiltDist, double twistDist, double symDist, double impropDist,
			double tiltAngle, double twistAngle,
			double minTtc, double maxTtc,
			double symTtc, double impropTtc,
			double disTtc,
			double disTiltAngle, double disTwistAngle,
									  boolean tiltMin, boolean twistMin, boolean symMin, boolean impropMin, 
									  boolean decomp, 
									  boolean ttc, boolean doSymTtc, 
									  boolean calcDisTtc, boolean decompDis, PointGroup ptGrp,
									  Matrix3x3[] setOfAllC) { 
		
		res = new TaskResultReprocess();
		...
		res.area = area;
		res.correlat = correlat;
		
		res.tiltDist = tiltDist;	
		res.twistDist = twistDist;
		res.symDist = symDist;
		res.impropDist = impropDist;
		res.tiltAngle = tiltAngle;
		res.twistAngle = twistAngle;
		
		res.minTtc = minTtc;
		res.maxTtc = maxTtc;
		
		res.symTtc = symTtc;
		res.impropTtc = impropTtc;
		
		res.disTtc = disTtc;
		
		res.disTiltAngle = disTiltAngle;
		res.disTwistAngle = disTwistAngle;
	...
	public Object call() throws Exception {
		...
		if(testTiltMin) res.tiltDist = Math.sqrt(res.tiltDist);
		if(testTwistMin) res.twistDist = Math.sqrt(res.twistDist);
		if(testSymMin) res.symDist = Math.sqrt(res.symDist);
		if(testImpropMin) res.impropDist = Math.sqrt(res.impropDist);
		
		if(testSymTTC) {
			res.symTtc = Math.sqrt(res.symTtc);
			res.impropTtc = Math.sqrt(res.impropTtc);
		}
