...
public final class GBPD_BinsWorker extends SwingWorker<Void,Void> {
	...
	public GBPD_BinsWorker(ArrayList<GBDatHeader> gbFiles,
	...
	public Void doInBackground() throws IOException {
		...
		out.println("# calculated using the method based on partition of the GB space into bins.");
		if(isExp) {
			out.println("EXP ");
		} else {
			out.println("RANDOM ");
		}			
		out.println("ST_PROJ_X ST_PROJ_Y ZENITH AZIMUTH MRD_PLANE");
		...
		System.out.println("GBPDWorker: ntocomput   " + NtoCompute);
				
		final double[][][][][] gbcd = new double[D1][D1][D1][D2][4*D2];
		final double[][] gbpd = new double[D2][4*D2];
		...
		for(int l = 0; l < D2; l++)
			for(int m = 0; m < 4*D2; m++) gbpd[l][m] = 0d;
		...
		//System.out.println("GBPDWorker: index   " + index);//TODO
	...
	//	System.out.println("GBPDWorker: index bis   " + index);//TODO
					...
					for(int k = 0; k < D1; k++)
						for(int l = 0; l < D2; l++)
							for(int m = 0; m < 4*D2; m++) {
								...
								final double temp = gbcd[i][j][k][l][m] / (D1*D1*D1);
								//System.out.println(i + " " + j + " " + k + " " + l + " "+m + "    " + temp);//TODO
								gbpd[l][m] += temp;
							}
			
			double avg = 0d;
			for(int l = 0; l < D2; l++)
				for(int m = 0; m < 4*D2; m++) avg += gbpd[l][m];
			
		//	System.out.println(avg);//TODO
			avg /= (4*D2*D2);
			//System.out.println(avg);//TODO
			for(int l = 0; l < D2; l++)
				for(int m = 0; m < 4*D2; m++) gbpd[l][m] /= avg;
		}
		
	//	System.out.println("GBPDWorker: a tu?");//TODO
		...
		for(int i = 0; i <= 30; i++) {
			for(int j = 0; j <= 120; j++) if(!isCancelled()) {
			    ...
			    for(Matrix3x3 C1 : setC) {
					...
					if(m1.z() < 0d) m1.negate();
						
					int i4 = (int) (D2 * FastMath.cos(m1.zenith()));									
					int i5 = (int) (4d * D2 * m1.azimuth() / TWOPI);
					...
					index++;
					//System.out.println("GBPDWorker: index   " + index + " " + NtoCompute);//TODO
		...
		//System.out.println("GBPDWorker: hmm?");//TODO
